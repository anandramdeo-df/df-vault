...
- <i> 1. Like in the below code snippet we are using `shareWithMe` controller type and you will get call back with `revokeData`, `assetID`, `newDate`. </i>
...
- <i> 2. View Controller type `organizationUploadAssetRrequestList` you need to use `DFDocument` framework to capture the document which user want to upload.  </i>
